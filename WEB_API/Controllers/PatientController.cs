    ...
    public class PatientController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllPatientTypes()
        {
            ...
            if (patientTypes == null || !patientTypes.Any())
            {
                return NotFound(new
                {
                    StatusCode = 404,Message = "No patient types found",Data = (object?)null
                });
            }

            return Ok(new
            {
                StatusCode = 200,Message = "Patient types retrieved successfully",Data = patientTypes
            });
        }
        ...
        public async Task<IActionResult> InsertPatientType([FromBody] PatientType patientType)
        {
            if (patientType == null || string.IsNullOrWhiteSpace(patientType.PatientTypeName))
            {
                ...
                {
                    StatusCode = 400,Message = "PatientTypeName is required",Data = (object?)null
                });
            }
            ...
            if (result > 0)
            {
                return Ok(new
                {
                    StatusCode = 200,Message = "Patient type inserted successfully",
                });
            }
            else
            {
                ...
                {
                    StatusCode = 500,Message = "Failed to insert patient type",Data = (object?)null
                });
            }
        }
        ...
        public async Task<IActionResult> DeletePatientType(int id)
        {
            if (id <= 0)
            {
                return BadRequest(new
                {
                    StatusCode = 400,Message = "Invalid PatientTypeID",Data = (object?)null
                });
            }
            ...
            if (result == 1)
            {
                return Ok(new
                {
                    StatusCode = 200,Message = "Patient type deleted successfully"
                });
            }
            else
            {
                return NotFound(new
                {
                    StatusCode = 404,Message = "Patient type not found",Data = (object?)null
                });
            }
        }
    }
